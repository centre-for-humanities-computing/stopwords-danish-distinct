...
class DistinctStopwords:
    ...
    def __init__(self,
        ...
        ----------
        target_lang : str
            ISO 639-1 code of language to get distinct stopwords for.

        n_stopwords : int
            How many distinct stopwords to get in target language.

        keep_lang : str|list
            Languages not to remove.

        unwanted_lang : str|list
            ISO 639-1 codes of languages to partly remove.
            _'all'_ uses all available languages,
            except for keep_lang & feared_lang.

        unwanted_threshold : int
            Use this many top words from an unwanted language to remove.

        feared_lang : str|list
            ISO 639-1 codes of languages to completely remove.
    ...
    def get_matching_paths(self, language_tags,
                           everything_except=False) -> list:
        '''
        Filter the list of files to get only those desired.

        Parameters
        ----------
        language_tags : str|list)
            tags of language to get filepaths for

        everything_except : bool
            if False, only paths specified by language_tags are extracted.
            if True, all paths everything except those in language_tags
            are extracted.
        '''
        ...
        if everything_except is False:
            pathlist = [
                path for path in self.word_paths if re.search(rpat, path)]

        elif everything_except is True:
            pathlist = [
                path for path in self.word_paths if not re.search(rpat, path)]

        return pathlist

    def get_distinct_stopwords(self):
        '''
        Find distinct words based on settings given in __init__
        '''
        ...
        if self.unwanted_lang == "all":
            ...
            except_feared = self.get_matching_paths(self.feared_lang,
                                                    everything_except=True)
            # inner join them into a single list
            unwanted_paths = list(set(except_wanted) & set(except_feared))
        ...
        # run through unwanted paths,
        # building a word dictionary for later removal.
        # feared is in front of unwanted,
        # so that it becomes the first added column
    ...
    def save(self):
        da_export = self.distinct_df.iloc[0:self.n_stopwords, :]
        da_export.word.to_csv(self.output_path, header=None,
                              index=None, sep=' ', mode='a')
        self.da_export = da_export
